...
# Neo4j auradb
# uri = "neo4j+s://fa52e4d3.databases.neo4j.io"
# user = "neo4j"
# password = "KjTxx8dGdoR0PvnM7RPhh3PDQvHUWRohnckjb3Rtxrw"
...
def extract_entities_structured(question: str) -> Entities:
    ...
    try:
        ...
        if json_match:
            ...
            if not case:
                case_match = re.search(r'([A-Z]+-\d+-\d{4}|\d+/\d{4}|[A-Z]+-\d{4,}-\d{4})', question, re.IGNORECASE)
...
def apply_reranker(query: str, chunks: List[dict]) -> List[dict]:
    try:
        ...
        logger.error(f"Error in FlashRank reranker: {str(e)}")
        return chunks   
...
SIMPLE_CYPHER_GENERATION_TEMPLATE = """You are a Neo4j Graph Database Expert. Given an input question, generate a simple Cypher query to retrieve relevant nodes and relationships. Use direct MATCH clauses based on the schema and avoid complex constructs unless necessary. Use specific relationships from the schema when possible instead of generic -[]-. If the previous attempt failed, adjust the query based on the feedback provided. Return only the Cypher query, nothing else.
...
Cypher query: MATCH (c:CASE {{Case_Number: 'crrfc 7'}})-[]-(lawyer:LAWYER) RETURN c AS nod, lawyer AS lawyer LIMIT 50

User input: Who is the judge in CR 221 2024
Cypher query: MATCH (c:CASE {{Case_Number: 'CR 221 2024'}})-[]-(judge:JUDGE) RETURN c AS nod, judge AS judge LIMIT 50

User input: Who is Karuna Gelhot
Cypher query: MATCH (n {{name: 'Karuna Gelhot'}}) RETURN n AS nod LIMIT 50

Feedback example: If no results: Try case-insensitive matching (e.g., toLower(n.name) CONTAINS 'value'), full-text indexes from the schema, alternative relationships (e.g., replace generic -[]- with schema-specific relationships), or different properties (e.g., name vs. alternative_name).
If syntax error: Check variable names, ensure labels and relationships exist in the schema (e.g., avoid typos in relationship names).
If irrelevant: Explore alternative labels, relationships, or broader matches (e.g., CONTAINS instead of exact match) from the schema.
...
def generate_simple_cypher(query, schema, feedback=None):
    try:
        prompt = SIMPLE_CYPHER_GENERATION_TEMPLATE.format(schema=schema, query=query)
        
        if feedback:
            # Sanitize feedback and limit length
            safe_feedback = feedback[:300].replace('\n', ' ')  # Truncate and remove newlines
            prompt += f"\n\nNote: The previous attempt failed: {safe_feedback}. Adjust the query by checking node labels, properties, or relationships in the schema. For example, if no nodes were found, try a broader match or a different label."
...
def graph_qa():
    ...
    while attempt < max_retries:
        try:
            ...
            if not chain_query:
                logger.debug("No valid simple Cypher query generated")
                break

            logger.debug(f"Simple cypher query chain_query: {chain_query}")
            context = graph.query(chain_query)
            logger.debug(f"Simple cypher query context: {context}")
            ...
            if chain_query and context:
                ...
                logger.debug(f"Simple cypher response result: {response_result}")
                logger.debug(f"Simple cypher relevance check: {relevance_result}")
                ...
                if relevance_result.is_relevant:
                    ...
                    logger.debug("Simple cypher output not relevant, falling back...")
                    feedback = f"Query '{chain_query}' returned irrelevant results or no matches. Check node labels, properties, or relationships."
                    last_query = chain_query
                    attempt += 1
                    continue
            else:
                feedback = f"Query '{chain_query}' returned empty results or no matching nodes. Try a broader match or different labels."
                last_query = chain_query
                attempt += 1

        except Exception as e:
            feedback = f"Execution error in query '{last_query}': {str(e)}. Adjust syntax or try a different approach."
            logger.error(f"Error in simple cypher query: {str(e)}")
            attempt += 1

        simple_cypher_time = time.time() - start_time
        logger.debug(f"Retrying simple cypher... Attempt {attempt}")
    ...
    while attempt < max_retries:
        try:
            ...
            if chain_query and context:
                ...
                logger.debug(f"Full-text cypher relevance check: {relevance_result}")
